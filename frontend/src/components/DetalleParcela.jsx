...
const DetalleParcela = ({ setVistaActual, parcelaId, esAdmin }) => {
    ...
    const cargarDetalles = async () => {
      try {
        const [resArea, resConfig, resMediciones, resHumedad, resNdvi] = await Promise.all([
          fetch(`${API_BASE_URL}/areas-riego/${parcelaId}`),
          fetch(`${API_BASE_URL}/configuraciones-cultivo`),
          fetch(`${API_BASE_URL}/mediciones-historicas`),
          fetch(`${API_BASE_URL}/mediciones-historicas/historial/${parcelaId}?limite=8`),
          fetch(`${API_BASE_URL}/mediciones-historicas/ndvi/${parcelaId}?dias=7`),
        ]);
        ...
        try { await fetch(`${API_BASE_URL}/alertas/verificar/${parcelaId}`); } catch {}
