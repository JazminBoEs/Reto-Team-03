...
const Sidebar = ({ vistaActual, setVistaActual, esAdmin, onLogout, alertasNoLeidas, usuarioActual }) => {
      ...
      {usuarioActual && (
        <div className="mb-6 px-2 py-3 bg-white/5 rounded-2xl border border-white/5">
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 bg-gradient-to-br from-creamy-blue to-emerald-600 rounded-full flex items-center justify-center text-white text-sm font-black shrink-0">
          ...
          {usuarioActual.predios && usuarioActual.predios.length > 0 && (
            <div className="mt-2 pt-2 border-t border-white/5">
              <p className="text-[10px] text-gray-500 uppercase tracking-widest mb-0.5">Predio</p>
              <p className="text-xs text-gray-300 font-medium truncate">{usuarioActual.predios[0].nombrePredio || 'Sin nombre'}</p>
            </div>
          )}
        </div>
      )}
