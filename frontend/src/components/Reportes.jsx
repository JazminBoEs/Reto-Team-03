...
const Reportes = ({ setVistaActual }) => {
  ...
  useEffect(() => {
    fetch(`${API_BASE_URL}/mediciones-historicas`)
      .then(res => res.json())
  ...
  return (
    <div className="animate-fade-in pb-10 max-w-6xl mx-auto">
      ...
      {/* FILTROS */}
      <div className="bg-earth-panel p-8 rounded-[2rem] border border-white/5 shadow-2xl mb-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        <div className="bg-[#0f2922] border border-emerald-500/20 p-6 rounded-[2rem] shadow-lg"><p className="text-gray-400 font-medium text-sm mb-2">Promedio Humedad</p><h3 className="text-4xl font-black text-emerald-400 mb-1">{promHumedad}%</h3><p className="text-xs text-gray-500 mt-2">Últimos {reportesFiltrados.length} registros</p></div>
        <div className="bg-[#2a1f13] border border-amber-500/20 p-6 rounded-[2rem] shadow-lg"><p className="text-gray-400 font-medium text-sm mb-2">Promedio Temp.</p><h3 className="text-4xl font-black text-[#f97316] mb-1">{promTemp}°C</h3><p className="text-xs text-gray-500 mt-2">Últimos {reportesFiltrados.length} registros</p></div>
        <div className="bg-[#0f2329] border border-cyan-500/20 p-6 rounded-[2rem] shadow-lg"><p className="text-gray-400 font-medium text-sm mb-2">Agua Usada</p><h3 className="text-4xl font-black text-[#00d1ff] mb-1">{aguaUsada.toLocaleString()}L</h3><p className="text-xs text-gray-500 mt-2">Acumulado total</p></div>
        <div className="bg-[#231533] border border-purple-500/20 p-6 rounded-[2rem] shadow-lg"><p className="text-gray-400 font-medium text-sm mb-2">Eficiencia Promedio</p><h3 className="text-4xl font-black text-[#a855f7] mb-1">{eficienciaGlobal}%</h3><p className="text-xs text-gray-500 mt-2">Basado en índice NDVI</p></div>
      </div>

      {/* RESUMEN */}
      <div className="bg-emerald-950/20 border border-emerald-500/30 p-8 rounded-[2rem] shadow-lg mb-8">
      ...
      {/* GRÁFICA */}
      <div className="bg-earth-panel p-8 rounded-[2rem] border border-white/5 shadow-2xl mb-8">
      ...
      {/* TABLA */}
      <div className="bg-earth-panel rounded-[2rem] border border-white/5 shadow-2xl overflow-hidden">
