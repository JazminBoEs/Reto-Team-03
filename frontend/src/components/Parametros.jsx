...
const Parametros = ({ setVistaActual, usuarioActual }) => {
    ...
    const cargar = async () => {
      try {
        const [rConfigs, rPredios, rAreas, rSensores] = await Promise.all([
          fetch(`${API_BASE_URL}/configuraciones-cultivo`).then(r => r.json()),
          fetch(`${API_BASE_URL}/predios`).then(r => r.json()),
          fetch(`${API_BASE_URL}/areas-riego`).then(r => r.json()),
          fetch(`${API_BASE_URL}/sensores`).then(r => r.json()),
        ]);
  ...
  const eliminarPredio = async (id) => {
    ...
    try {
      const res = await fetch(`${API_BASE_URL}/predios/${id}`, { method: 'DELETE' });
  ...
  const eliminarArea = async (id) => {
    ...
    try {
      const res = await fetch(`${API_BASE_URL}/areas-riego/${id}`, { method: 'DELETE' });
  ...
  const guardarConfig = async (data) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (areaId) {
          try { await fetch(`${API_BASE_URL}/alertas/verificar/${areaId}`); } catch {}
        }
      } else { const err = await res.json(); mostrarMensaje('error', err.message || 'Error'); }
    } catch { mostrarMensaje('error', 'Error de conexión'); }
  };
  ...
  const eliminarConfig = async (id) => {
    ...
    try {
      const res = await fetch(`${API_BASE_URL}/configuraciones-cultivo/${id}`, { method: 'DELETE' });
...
const ModalFormulario = ({ modal, setModal, predios, areas, onGuardarPredio, onGuardarArea, onGuardarConfig }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={() => setModal(null)}>
      <div className="bg-earth-panel border border-white/10 rounded-[2rem] p-8 w-full max-w-lg shadow-2xl animate-fade-in" onClick={e => e.stopPropagation()}>
        <div className="flex justify-between items-center mb-6">
