...
// ─── Main Component ────────────────────────────────────────
export default function MapaDePredio({ setVistaActual, setParcelaActiva }) {
  ...
  const fetchData = async () => {
    ...
    try {
      const [prediosRes, sensoresRes, areasRes] = await Promise.all([
        fetch(`${API_BASE}/predios`),
        fetch(`${API_BASE}/sensores`),
        fetch(`${API_BASE}/areas-riego`),
      ]);
    ...
    <div className="animate-fade-in">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-5 mb-6">
        <div className="bg-earth-panel p-5 rounded-3xl border border-white/5 shadow-xl flex items-center gap-4">
          ...
          <div>
            <p className="text-gray-400 font-medium uppercase text-[10px] tracking-[0.1em]">
              Predios
            </p>
      ...
      <div className="flex gap-6">
        {/* Map Container */}
        <div className="flex-1 bg-earth-panel rounded-[2rem] border border-white/5 shadow-2xl overflow-hidden relative">
          {/* Loading overlay */}
          {loading && (
            ...
            <div className="space-y-3">
              {predios.length > 0 ? (
                predios.map((predio) => (
                  <div
                    key={predio.IDpredio}
                    className="flex items-center gap-3 p-3 rounded-2xl bg-white/[.03] border border-white/5 hover:bg-white/[.06] transition-colors"
                  >
